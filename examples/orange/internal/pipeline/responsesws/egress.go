...
const EgressFilterName = "orange-responsesws-egress-match"

// internalHeaders is the complete list of x-orange-responsesws-* headers that must be
...
// egressHandler is the orange-responsesws-egress-match downstream filter handler.
//
// It runs on the Envoy egress listener before the router. Its responsibilities:
//   - Validate the four x-orange-responsesws-* headers written by the orange-responsesws sidecar.
...
//   - Strip all x-orange-responsesws-* headers unconditionally before the upstream route.
//
...
func egressHandler(w *up.Writer, r *up.Request) {
	...
	if provider == "" || kind == "" || model == "" || backendModel == "" {
		send.Error(w, http.StatusBadRequest, send.InvalidRequestError,
			"orange.responsesws_headers_missing",
			"orange-responsesws internal headers are missing; request did not originate from orange-responsesws sidecar")
		return
	}
	...
	if configProvider != provider {
		send.Errorf(w, http.StatusBadRequest, send.InvalidRequestError,
			...
			"x-orange-responsesws-provider %q does not match active config provider %q for model %q",
