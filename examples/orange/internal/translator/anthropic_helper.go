...
func validateTemperatureForAnthropic(temp *float64) error {
	if temp != nil && (*temp < 0.0 || *temp > 1.0) {
		return fmt.Errorf("%w: "+tempNotSupportedError, ErrInvalidRequestBody, *temp)
	}
	return nil
}
...
func modelContainsAny(model string, identifiers []string) bool {
...
func outputConfigAvailable(model string) bool {
...
func effortAvailable(model string) bool {
...
func mapReasoningEffortToOutputConfigEffort(reasonEffort openaisdk.ReasoningEffort) (anthropic.OutputConfigEffort, error) {
	...
	default:
		return "", fmt.Errorf("%w: unsupported reasoning effort level: %q (supported: low, medium, high, xhigh)", ErrInvalidRequestBody, reasonEffort)
	}
}
...
func buildAnthropicParams(openAIReq *openai.ChatCompletionRequest, apiSchema string, modelNameOverride string) (params *anthropic.MessageNewParams, err error) {
...
type anthropicStreamParser struct {
	...
	toolIndex       int64
	// CODEMOD-TODO: token usage dropped (metrics.TokenUsage removed)
...
func (p *anthropicStreamParser) Process(body io.Reader, endOfStream bool) (
	newHeaders []Header, newBody []byte, responseModel string, err error,
) {
	newBody = make([]byte, 0)
	...
	if endOfStream {
		// CODEMOD-TODO: token usage dropped
		...
		if len(finalChunk.Choices) > 0 {
			...
			if err != nil {
				return nil, nil, "", fmt.Errorf("failed to marshal final stream chunk: %w", err)
			}
		}
...
func messageToChatCompletion(anthropicResp *anthropic.Message, responseModel string) (openAIResp *openai.ChatCompletionResponse, err error) {
	...
	for i := range anthropicResp.Content { // NOTE: Content structure is massive, do not range over values.
		...
		case string(constant.ValueOf[constant.ToolUse]()):
			if output.ID != "" {
				...
				if toolErr != nil {
					return nil, fmt.Errorf("failed to convert anthropic tool use to openai tool call: %w", toolErr)
				}
