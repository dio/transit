...
import (
	...
	// MANUAL-FIX: gcp import was wrong envoyproxy path; corrected to transit local package.
	"github.com/dio/transit/examples/orange/internal/apischema/gcp"
...
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) RequestBody(raw []byte) (newHeaders []Header, newBody []byte, err error){
...
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) ResponseHeaders(_ map[string]string) (newHeaders []Header, err error) {
	if o.stream {
...
// MANUAL-FIX: removed `span` arg from handleStreamingResponse; fixed undefined `responseModel`.
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) ResponseBody(chunk []byte, endOfStream bool) (newHeaders []Header, newBody []byte, err error) {
	if o.stream {
		return o.handleStreamingResponse(bytes.NewReader(chunk), endOfStream)
	}

	// Non-streaming logic.
	gcpResp := &genai.GenerateContentResponse{}
	if err = json.NewDecoder(bytes.NewReader(chunk)).Decode(gcpResp); err != nil {
		return nil, nil, fmt.Errorf("error decoding GCP response: %w", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("error converting GCP response to OpenAI format: %w", err)
	}

	// Marshal the OpenAI response.
	newBody, err = json.Marshal(openAIResp)
	if err != nil {
		return nil, nil, fmt.Errorf("error marshaling OpenAI response: %w", err)
	}
...
// MANUAL-FIX: broken codemod output replaced; removed span/tokenUsage params from signature.
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) handleStreamingResponse(body io.Reader, endOfStream bool) (
	...
	if err != nil {
		return nil, nil, fmt.Errorf("error parsing GCP streaming chunks: %w", err)
	}
	...
	for i := range chunks {
		...
		if serErr := serializeOpenAIChatCompletionChunk(openAIChunk, &newBody); serErr != nil {
			return nil, nil, fmt.Errorf("error marshaling OpenAI chunk: %w", serErr)
		}
		...
		if chunk.UsageMetadata != nil && chunk.UsageMetadata.PromptTokenCount > 0 {
			...
			// Serialize to SSE format as expected by OpenAI API.
			if serErr := serializeOpenAIChatCompletionChunk(usageChunk, &newBody); serErr != nil {
				return nil, nil, fmt.Errorf("error marshaling OpenAI usage chunk: %w", serErr)
			}
			// CODEMOD-TODO: wire token usage via Envoy stats
		}
	}
	...
	// format body unchanged in STREAMED mode.
	if newBody == nil {
		newBody = []byte{}
	}
...
// MANUAL-FIX: argsMarshaller type changed from json.Marshaler (interface) to jsonMarshalerFunc (func type).
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) extractToolCallsFromGeminiPartsStream(
	...
	argsMarshaller jsonMarshalerFunc,
...
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) openAIMessageToGeminiMessage(openAIReq *openai.ChatCompletionRequest, requestModel string) (*gcp.GenerateContentRequest, error) {
...
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) applyVendorSpecificFields(openAIReq *openai.ChatCompletionRequest, gcr *gcp.GenerateContentRequest, requestModel string) {
...
// MANUAL-FIX: redaction package dropped; stub returns msg unchanged.
...
func (o *openAIToGCPVertexAITranslatorV1ChatCompletion) RequestHeaders(_ map[string]string) ([]Header, error) {
	return nil, nil
}

func init() {
	Register("gcpvertexai", func(cfg ProviderConfig) Translator {
		return NewChatCompletionOpenAIToGCPVertexAITranslator(cfg)
	})
}
