...
func NewChatCompletionOpenAIToAWSBedrockTranslator(cfg ProviderConfig) Translator{
	return &openAIToAWSBedrockTranslatorV1ChatCompletion{backendModel: cfg.BackendModel}
}
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) RequestBody(raw []byte) (newHeaders []Header, newBody []byte, err error){
	...
	// MANUAL-FIX: methods take *openai.ChatCompletionRequest, not value.
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) openAIToolsToBedrockToolConfiguration(openAIReq *openai.ChatCompletionRequest,
	...
	if openAIReq.ToolChoice != nil {
		if toolChoice, ok := openAIReq.ToolChoice.Value.(string); ok {
			...
			return fmt.Errorf("%w: tool_choice type not supported", ErrInvalidRequestBody)
		}
	}
	return nil
}
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) openAIMessageToBedrockMessageRoleUser(
		...
		for i := range contents {
			...
			if contentPart.OfText != nil {
				...
				case mimeTypeImageWEBP:
					format = "webp"
				default:
					return nil, fmt.Errorf("%w: unsupported image format %s", ErrInvalidRequestBody, contentType)
				}
	...
	return nil, fmt.Errorf("%w: unexpected content type for user message", ErrInvalidRequestBody)
}
	...
	for _, content := range contentParts {
		...
		case openai.ChatCompletionAssistantMessageParamContentTypeRedactedThinking:
			if content.RedactedContent != nil {
				...
				case string:
					return nil, fmt.Errorf("%w: redacted_content must be a binary/bytes value in bedrock", ErrInvalidRequestBody)
				default:
					return nil, fmt.Errorf("%w: redacted_content must be a binary/bytes value in bedrock", ErrInvalidRequestBody)
				}
			}
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) openAIMessageToBedrockMessageRoleSystem(
	...
	if v, ok := openAiMessage.Content.Value.(string); ok {
		*bedrockSystem = append(*bedrockSystem, &awsbedrock.SystemContentBlock{
		...
		return fmt.Errorf("%w: unexpected content type for system message", ErrInvalidRequestBody)
	}
	return nil
}
		...
		case msg.OfDeveloper != nil:
			...
			if text, ok := message.Content.Value.(string); ok {
				...
				if contents, ok := message.Content.Value.([]openai.ChatCompletionContentPartTextParam); ok {
					...
					return fmt.Errorf("%w: unexpected content type for developer message", ErrInvalidRequestBody)
				}
			}
		case msg.OfTool != nil:
			...
			bedrockReq.Messages = append(bedrockReq.Messages, bedrockMessage)
		default:
			return fmt.Errorf("%w: unexpected role: %s", ErrInvalidRequestBody, msg.ExtractMessgaeRole())
		}
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) ResponseHeaders(headers map[string]string) (newHeaders []Header, err error) {
	if o.stream {
		...
		if contentType == "application/vnd.amazon.eventstream" {
			...
			newHeaders = []Header{{contentTypeHeaderName, "text/event-stream"}}
		}
	}
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) ResponseBody(chunk []byte, endOfStream bool) (newHeaders []Header, newBody []byte, err error){
	if o.stream {
		newBody = make([]byte, 0)
		var buf []byte
		...
		for i := range o.events {
			...
			if usage := event.Usage; usage != nil {
				// MANUAL-FIX: metrics.ExtractTokenUsageFromExplicitCaching dropped; token usage not wired.
				_ = usage
			}
	...
	if err = json.NewDecoder(bytes.NewReader(chunk)).Decode(&bedrockResp); err != nil {
	...
	// Convert token usage.
	// MANUAL-FIX: metrics.ExtractTokenUsageFromExplicitCaching dropped; compute directly from bedrock fields.
	if bedrockResp.Usage != nil {
		openAIResp.Usage = openai.Usage{
			TotalTokens:      int(bedrockResp.Usage.TotalTokens),
			PromptTokens:     int(bedrockResp.Usage.InputTokens),
			CompletionTokens: int(bedrockResp.Usage.OutputTokens),
		}
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) convertEvent(event *awsbedrock.ConverseStreamEvent) (*openai.ChatCompletionResponseChunk, bool) {
	...
	case awsbedrock.ConverseStreamEventTypeMetadata.String():
		...
		// MANUAL-FIX: metrics.ExtractTokenUsageFromExplicitCaching dropped; compute directly from event fields.
...
func (o *openAIToAWSBedrockTranslatorV1ChatCompletion) RequestHeaders(_ map[string]string) ([]Header, error) {
	return nil, nil
}
