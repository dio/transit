...
func (o *openAIToOpenAITranslatorV1ChatCompletion) RequestBody(raw []byte) (newHeaders []Header, newBody []byte, err error) {
	...
	if req.Stream {
		// Inject stream_options.include_usage so the final SSE chunk carries
		// token counts for metering. Safe to set even when the client already
		// included it; sjson overwrites the field in-place.
		src := raw
		if len(newBody) > 0 {
			src = newBody
		}
		newBody, err = sjson.SetBytesOptions(src, "stream_options.include_usage", true, sjsonOptions)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to set stream_options: %w", err)
		}
	}
