...
import (
	...
	"github.com/dio/transit/examples/orange/translate"
)
...
func TestRouteFor_pathRewrite_appliedWhenPrefixDiffers(t *testing.T) {
	h, w, r := newHandle(map[string]string{":method": "POST", ":path": "/v1/messages"})
	route := translate.RouteFor(translate.SchemaOpenAI, translate.ProviderConfig{
		Schema:     translate.SchemaOpenAI,
		PathPrefix: "/api/v1",
		Secret:     "tok",
...
func TestRouteFor_noPathRewrite_whenPrefixIsV1(t *testing.T) {
	h, w, r := newHandle(map[string]string{":method": "POST", ":path": "/v1/chat/completions"})
	route := translate.RouteFor(translate.SchemaOpenAI, translate.ProviderConfig{
		Schema:     translate.SchemaOpenAI,
		PathPrefix: "/v1",
		Secret:     "tok",
