...
func TestEnvoyGatewayInstallOnly(t *testing.T) {
	if os.Getenv("RUN_MCP_PROFILE_TIERED_ROUTER_EG_INSTALL") != "1" {
		t.Skip("set RUN_MCP_PROFILE_TIERED_ROUTER_EG_INSTALL=1 to run the Envoy Gateway install smoke test")
	}
	suite.Run(t, &envoyGatewayInstallSuite{
		envoyGatewaySuite: envoyGatewaySuite{
			cluster: "transit-mcp-profile-eg-install",
